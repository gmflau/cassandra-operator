...
import (
	...
	"github.com/gmflau/cassandra-operator/pkg/chaos"
	"github.com/gmflau/cassandra-operator/pkg/client"
	"github.com/gmflau/cassandra-operator/pkg/controller"
	"github.com/gmflau/cassandra-operator/pkg/debug"
	...
	"github.com/gmflau/cassandra-operator/version"
...
func init() {
	flag.StringVar(&debug.DebugFilePath, "debug-logfile-path", "", "only for a self hosted cluster, the path where the debug logfile will be written, recommended to be under: /var/tmp/cassandra-operator/debug/ to avoid any issue with lack of write permissions")
	...
	flag.IntVar(&chaosLevel, "chaos-level", -1, "DO NOT USE IN PRODUCTION - level of chaos injected into the cassandra clusters created by the operator.")
...
func main() {
	...
	if printVersion {
		fmt.Println("cassandra-operator Version:", version.Version)
	...
	kubecli := k8sutil.MustNewKubeClient(os.Getenv("KUBECONFIG"))
...
func newControllerConfig() controller.Config {
	...
	cfg := controller.Config{
		...
		KubeCli:        kubecli,
		KubeExtCli:     k8sutil.MustNewKubeExtClient(os.Getenv("KUBECONFIG")),
		CassandraCRCli:      client.MustNewInCluster(),
	}
...
func startChaos(ctx context.Context, kubecli kubernetes.Interface, ns string, chaosLevel int) {
	...
	ls := labels.SelectorFromSet(map[string]string{"app": "cassandra"})

	switch chaosLevel {
	case 1:
		logrus.Info("chaos level = 1: randomly kill one cassandra pod every 30 seconds at 50%")
		c := &chaos.CrashConfig{
			Namespace: ns,
			Selector:  ls,
	...
	case 2:
		logrus.Info("chaos level = 2: randomly kill at most five cassandra pods every 30 seconds at 50%")
		c := &chaos.CrashConfig{
			Namespace: ns,
			Selector:  ls,
