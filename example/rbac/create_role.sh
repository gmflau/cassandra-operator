...
echo ETCD_OPERATOR_ROOT $ETCD_OPERATOR_ROOT

print_usage() {
  echo "$(basename "$0") - Create Kubernetes RBAC role and role bindings for cassandra-operator
Usage: $(basename "$0") [options...]
Options:
  --role-name=STRING         Name of ClusterRole to create
                               (default=\"cassandra-operator\", environment variable: ROLE_NAME)
  --role-binding-name=STRING Name of ClusterRoleBinding to create
                               (default=\"cassandra-operator\", environment variable: ROLE_BINDING_NAME)
...
ROLE_NAME="${ROLE_NAME:-cassandra-operator}"
ROLE_BINDING_NAME="${ROLE_BINDING_NAME:-cassandra-operator}"
...
sed -e "s/<ROLE_NAME>/${ROLE_NAME}/g" \
  ...
  kubectl create --username=admin --password=EJHmxMkMxe0CSdeC -f -
