...
type Config struct {
	...
	CassandraCRCli versioned.Interface
}
...
func New(config Config, cl *api.CassandraCluster) *Cluster {
...
func (c *Cluster) ResolvePodServiceAddress(member *cassandrautil.Member) (string, error) {
	contactPoint := ""
...
func (c *Cluster) setup() error {
	...
	if c.isSecureClient() {
		...
		c.tlsConfig, err = cassandrautil.NewTLSConfig(d.CertData, d.KeyData, d.CAData)
		if err != nil {
			return err
		}
	}
...
func (c *Cluster) run() {
	...
	for {
		...
		case <-time.After(reconcileInterval):
			...
			if len(running) == 0 {
				c.logger.Warningf("all cassandra pods are dead. Trying to recover from a previous backup")
...
func (c *Cluster) startSeedMember(recoverFromBackup bool) error {
	m := &cassandrautil.Member{
		Name:         cassandrautil.CreateMemberName(c.cluster.Name, c.memberCounter),
	...
	if ip, err := c.ResolvePodServiceAddress(m); err != nil {
		m.IP = ip
	}
	c.memberCounter++
	c.members = ms
...
func (c *Cluster) Update(cl *api.CassandraCluster) {
	c.send(&clusterEvent{
...
func (c *Cluster) createPod(members cassandrautil.MemberSet, m *cassandrautil.Member, state string, needRecovery bool) error {
	pod := k8sutil.NewCassandraPod(m, members.ExcludedSeeds(m.Name), c.cluster.Name, state, c.cluster.Spec, c.cluster.AsOwner())
	//if needRecovery {
...
func (c *Cluster) updateMemberStatus(members cassandrautil.MemberSet) {
	...
	for _, m := range members {
		url := m.Addr()
		ip, _ := c.ResolvePodServiceAddress(m) //TODO: make IP Detection a bit better
		healthy, err := cassandrautil.CheckHealth(ip, c.tlsConfig)
		if err != nil {
...
func (c *Cluster) updateCRStatus() error {
	...
	newCluster, err := c.config.CassandraCRCli.CassandraV1beta2().CassandraClusters(c.cluster.Namespace).Update(c.cluster)
	if err != nil {
...
func (c *Cluster) reportFailedStatus() {
	...
	f := func() (bool, error) {
		...
		cl, err := c.config.CassandraCRCli.CassandraV1beta2().CassandraClusters(c.cluster.Namespace).
