...
type EtcdClusterCRUpdateFunc func(*api.CassandraCluster)

func GetClusterList(restcli rest.Interface, ns string) (*api.CassandraClusterList, error) {
	b, err := restcli.Get().RequestURI(listClustersURI(ns)).DoRaw()
	if err != nil {
		return nil, err
	}

	clusters := &api.CassandraClusterList{}
...
func MustNewKubeExtClient(kubeconfig string) apiextensionsclient.Interface {
