...
func NewMemberAddEvent(memberName string, cl *api.CassandraCluster) *v1.Event {
...
func MemberRemoveEvent(memberName string, cl *api.CassandraCluster) *v1.Event {
	event := newClusterEvent(cl)
	event.Type = v1.EventTypeNormal
...
func ReplacingDeadMemberEvent(memberName string, cl *api.CassandraCluster) *v1.Event {
	event := newClusterEvent(cl)
	event.Type = v1.EventTypeNormal
...
func MemberUpgradedEvent(memberName, oldVersion, newVersion string, cl *api.CassandraCluster) *v1.Event {
	event := newClusterEvent(cl)
	event.Type = v1.EventTypeNormal
...
func newClusterEvent(cl *api.CassandraCluster) *v1.Event {
	t := time.Now()
	return &v1.Event{
